...
def multilabel_accuracy(y_true: Union[np.ndarray, torch.Tensor], 
    ...
    Example:
        ...
        0.6666666666666666  # 3个样本中2个完全正确
    """
    # 转换为numpy数组
...
if __name__ == "__main__":
    ...
    # 示例数据
    y_true = np.array([
        [1, 0, 1, 0, 0, 0],  # 样本1：rust, frog_eye_leaf_spot
        [0, 1, 0, 0, 0, 0],  # 样本2：complex
        [1, 1, 0, 0, 0, 0],  # 样本3：rust, complex
        [0, 0, 1, 0, 0, 0],  # 样本4：healthy
    ])
    
    y_pred = np.array([
        [1, 0, 1, 0, 0, 0],  # 完全正确
        [0, 1, 0, 0, 0, 1],  # 错误（多预测了一个标签）
        [1, 1, 0, 0, 0, 0],  # 完全正确
        [0, 0, 1, 0, 0, 0],  # 完全正确
    ])
    
    accuracy = multilabel_accuracy(y_true, y_pred)
    print(f"多标签准确率: {accuracy:.4f} (应该是0.75，4个样本中3个完全正确)")
    
    # 计算所有指标
    metrics = calculate_all_metrics(y_true, y_pred)
    print_metrics_summary(metrics, "测试指标")
